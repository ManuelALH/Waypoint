...
def get_system_schema(system_slug):
    try:
        system_obj = GameSystem.objects.get(slug=system_slug)
        schema = system_obj.schema_definition
        
        if 'meta' not in schema:
            schema['meta'] = {}
        
        schema['meta']['color'] = system_obj.primary_color
        schema['meta']['label'] = system_obj.name
